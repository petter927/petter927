[📄 下載完整履歷 (若無法預覽請點此)](https://raw.githubusercontent.com/petter927/petter927/main/Resume_PetterKu.pdf)

# Hi, I'm Petter Ku (顧元暉) 👋
### 跨領域開發者 | 13年外商廠務經驗 x C# .NET / SAP ABAP 系統開發

> 「始終保持根本目的思考，在時效與預算內達成目標。」

---

## 🏆 專業證照 (Certifications)
* **IT Specialist: Software Development** (2024.10)
* **IT Specialist: Databases** (2024.10)
* **IT Specialist: Python** (2025.08)
* **IT Specialist: AI** (2024.08)
* **勞動部職訓結訓**：程式開發 (C#、Python) 與 ERP 系統設計
* **ERP 軟體應用師：鼎新 Workflow ERP GP (製造模組)** | 中華企業資源規劃學會 (2024.10)
* **ERP 軟體應用師：鼎新 Workflow ERP GP (銷售模組)** | 中華企業資源規劃學會 (2024.07)
* **ERP 軟體應用師：鼎新 Workflow ERP GP (財務模組)** | 中華企業資源規劃學會 (2024.08)

## 🛠 核心技術棧 (Tech Stack)
* **Languages**: C# (.NET Core/MVC/WinForm), SAP ABAP, Python, SQL
* **Web**: HTML5, CSS3, JavaScript, jQuery, Razor
* **Database**: MS-SQL (EF Core/ADO.NET), MySQL
* **ERP**: SAP PM/MM 模組操作、BAPI 整合開發、鼎新 WorkFlow

## 💼 核心工作經歷 (Experience)
#### **阿特拉斯科普柯 (Atlas Copco) | 產品售後技術服務 Planner**
*2025.05 - 2025.08*
* 管理 **SAP ERP 工單** 與安排計劃，確保及時達成部門目標。
* 維護 SAP ERP 派工系統，確保工作均正確分配。

#### **台灣科幕 (Chemours) / 杜邦 (DuPont) | 儀電 PM Planner / 資訊專員**
*2010.07 - 2024.03*
* **IT 支援**：負責機房設備、電話及網路系統維護與故障排除。
* **維護計畫**：利用 SAP PM/MM 模組管理備品、採購及 BOM 資料。

#### **奧圖美德 (Automated) | 電腦服務工程師**
*2007.03 - 2010.07*
* 負責客戶端電腦硬體維修、作業系統安裝及網路環境建置。
* 累積許多的電腦故障診斷與技術服務經驗。

---

## 💻 專案作品 (Portfolio)
* **[HRMS-CSharp](https://github.com/petter927/HRMS-CSharp)**: 採用 **Cookie-based Authentication** 與 **Role-based** 權限管理之員工出勤系統。
* **SAP ABAP 整合**: 利用 **Module Pool** 與 **BAPI** 實作互動式請購單管理工具。

---
[📄 下載完整履歷 (PDF)](https://raw.githubusercontent.com/petter927/petter927/main/Resume_PetterKu.pdf)
📫 **Email**: petter.ku@yahoo.com | 📍 **Location**: 桃園市中壢區